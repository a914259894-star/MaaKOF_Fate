...
- picks 填 **[栏,行,列]**（1 起，栏 1~10），对应 `image/arena/roles/c{栏}r{行}k{列}.png`。
  ...
  **执行顺序**：开始前回卷到最左（游戏会记住上次浏览位置），再按栏号升序找人
  （你填的先后无关，游戏不关心选人顺序）。拖到最后一栏画面不再变化即停，不会傻拖满 6 屏。
  控制台每屏打印最高分，对不上号直接对照日志。
  ...
  **补裁须知**：选人界面的格子停靠位置每次截图有 ±30px 横向漂移（3.png 实测 dx=32、
  ...
  examples/roles_overview_90.png 是全部 90 张的验收图，换账号/大版本更新后重新对照。
